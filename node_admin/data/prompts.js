/**
 * AI提示词定义
 * 从原Python项目中移植并适配Node.js环境
 */

module.exports = {
  // 核心种子设定提示词
  coreSeedPrompt: `作为专业作家，请用"雪花写作法"第一步构建故事核心：
主题：{topic}
类型：{genre}
...
仅返回故事核心文本，不要解释任何内容。`,
  ...
  // 角色动力学设定提示词
  characterDynamicsPrompt: `基于以下元素：
- 内容指导：{user_guidance}
- 核心种子：{core_seed}
...
仅给出最终文本，不要解释任何内容。`,
  ...
  // 章节摘要生成提示词
  summarizeRecentChaptersPrompt: `作为一名专业的小说编辑和知识管理专家，正在基于已完成的前三章内容和本章信息生成当前章节的精准摘要。请严格遵循以下工作流程：
前三章内容：
{combined_text}
...
仅返回前文摘要文本，不要解释任何内容。`,
  ...
  // 角色状态创建提示词
  createCharacterStatePrompt: `依据当前角色动力学设定：{character_dynamics}
...
仅返回编写好的角色状态文本，不要解释任何内容。`,
  ...
  // 角色状态更新提示词
  updateCharacterStatePrompt: `以下是新完成的章节文本：
{chapter_text}
...
仅返回更新后的角色状态文本，不要解释任何内容。`,
  ...
  // 第一章草稿生成提示词
  firstChapterDraftPrompt: `【重要】你正在创作小说的第一章，请严格遵守以下要求：
...
【创作要求】
1. 只专注于第一章内容的创作，绝对不要提前展开后续章节的情节
2. 完成第 {novel_number} 章的正文，字数要求{word_number}字
3. 至少设计下方2个或以上具有动态张力的场景：
...
- 不使用分章节小标题；
- 不要使用markdown格式；
- 不要出现"第二章"、"后续"等指向未来章节的词语；
- 确保内容在第一章范围内完整，不要留下"未完待续"的暗示。

【额外指导】(可能未指定)：{user_guidance}

【警告】
绝对不要提前展开后续章节的情节！只创作第一章内容！`,

  // 后续章节草稿生成提示词
  nextChapterDraftPrompt: `参考文档：
└── 前文摘要：
    {global_summary}
